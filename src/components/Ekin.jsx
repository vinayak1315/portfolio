...
const Ekin = ({ onPrev, onNext }) => {
  ...
  return (
    <div className="m-auto mb-20 md:mb-[165px] px-4 md:px-0" style={{ fontFamily: 'Poppins, sans-serif', maxWidth: '1100px' }}>
      <motion.div style={{ fontFamily: "'Bestie Seventy', cursive" }} className="text-[28px] md:text-[48px] font-medium flex flex-col items-center" {...fadeUp(0)}>Ekin Branding</motion.div>
      ...
      <motion.div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-3 md:mt-[48px] w-full gap-1 sm:gap-0" {...fadeUp(0.18)}>
        <div className="text-[14px] sm:text-[20px] font-medium">Client: <span className="font-bold ml-[8px]">EKIN</span></div>
      ...
      <motion.div className="relative mt-3 md:mt-[31px]" {...fadeUp(0.28)}>
        <div className="flex flex-col md:flex-row gap-6 md:gap-[40px] w-full">
          ...
          <div className="w-full md:w-[45%]">
            <ImageWithHover src={ekin_01} alt="Ekins" style={{ height: '247px', marginTop: '17px' }} onClick={() => setPopup('image1')} icon="🔍" label="View Image" />
            <div className="w-full flex justify-between mt-[34px] gap-[26px]">
              <ImageWithHover src={ekin_02} alt="Ekins" style={{ width: '50%', height: '138px' }} onClick={() => setPopup('image2')} icon="🔍" label="View Image" />
              <ImageWithHover src={ekin_03} alt="Ekins" style={{ width: '50%', height: '138px' }} onClick={() => setPopup('image3')} icon="🔍" label="View Image" />
            </div>
            <div className="w-full flex justify-between mt-[34px] gap-[26px]">
              <ImageWithHover src={ekin_04} alt="Ekins" style={{ width: '50%', height: '138px' }} onClick={() => setPopup('image4')} icon="🔍" label="View Image" />
              <ImageWithHover src={ekin_05} alt="Ekins" style={{ width: '50%', height: '138px' }} onClick={() => setPopup('image5')} icon="🔍" label="View Image" />
            </div>
          </div>
          ...
          <div className="w-full md:w-[55%] bg-[#95BBEA] relative border-[5px] text-[16px] border-[#003049] px-6 md:px-[51px] py-6 md:py-[52px]" style={{ boxShadow: '5px 7px 7.2px 0 rgba(0, 0, 0, 0.79)', borderRadius: "20px" }}>
            ...
            <p style={{ fontFamily: 'Poppins, sans-serif' }} className="font-medium">
              A new brand of sneakers launched for the non-sporty, not in perfect shape millions. The brand's mission is to make the lazy/not so fit folks feel great about themselves.
            </p>
            <br />
        ...
        <div onClick={e => e.stopPropagation()} style={{ position: 'relative', maxWidth: '80vw', maxHeight: '85vh', transform: popup ? 'scale(1)' : 'scale(0.88)', transition: 'transform 0.3s ease' }}>
          ...
          {popup && (
            <div style={{ width: '75vw', height: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src={{ image1: ekin_01, image2: ekin_02, image3: ekin_03, image4: ekin_04, image5: ekin_05 }[popup]} alt="Popup" style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
            </div>
          )}
        </div>
      </div>, document.body)}
